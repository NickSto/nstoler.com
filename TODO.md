...
- Bug: page won't reload if the url doesn't change
  - Affects when user hits "select all" or "select none", changes a few checkboxes, and hits the same button again.
  - Workaround w/o JavaScript is probably to add a "reload=" parameter, with a randomized value generated by the view.
